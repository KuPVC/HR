<template>
	<div class="w-full max-w-3xl mx-auto">
		<div
			class="flex flex-row bg-white shadow-sm py-4 px-3 items-center justify-between border-b rounded-t"
		>
		...
		>
			<div class="w-full">
				...
				<div v-if="hasNextPage && documents.data?.length" class="mt-3">
					<Button variant="subtle" class="w-full" @click="fetchDocumentList(documents.params.start + listOptions.page_length)">
						{{ __("Load more") }}
					</Button>
				</div>
			</div>
		...
		<!-- Filter dialog -->
		<Dialog v-model="showFilterDialog" :options="{ title: __('Filters') }">
...
import { FeatherIcon, createResource, LoadingIndicator, debounce, Dialog } from "frappe-ui"
